...
routes.get('/equipo', (req, res) => {
    req.getConnection((err, conn) => {
        ...
        const query = `
            SELECT * FROM (
                SELECT equipo.*, estacion_trabajo.nombre_estacion
                FROM equipo
                JOIN estacion_trabajo ON equipo.COD_ESTACION = estacion_trabajo.COD_ESTACION
                WHERE equipo.TIPO = "Portatil" AND equipo.ESTADO != "alta"
                UNION
                SELECT *, NULL
                FROM equipo
                WHERE equipo.TIPO = "Escritorio" AND equipo.ESTADO != "alta"
            ) AS equipo_tipo
            ORDER BY equipo_tipo.MARCA ASC;
        `;
...
routes.get('/equiposDanados', (req, res)=>{
    req.getConnection((err, conn)=>{
        ...
        conn.query('SELECT * FROM equipo WHERE ESTADO = ? OR ESTADO = ? ORDER BY equipo.COD_EQUIPO' , ['dañado','Dañado'], (err, rows)=>{
            if(err) return res.send(err)
            res.json(rows)
        })
    })
...
routes.get('/equipoRefactorizar', (req, res)=>{
    req.getConnection((err, conn)=>{
        ...
        conn.query('SELECT * FROM equipo WHERE ESTADO = ? OR ESTADO = ? ORDER BY equipo.COD_EQUIPO  ', ['repotenciar', 'Repotenciar'], (err, rows)=>{
            if(err) return res.send(err)
            res.json(rows)
        })
    })
...
routes.get('/equipo/:COD_ESTACION', (req, res) => {
    ...
    req.getConnection((err, conn) => {
        ...
        const query = `SELECT * FROM equipo WHERE COD_ESTACION = ? AND equipo.ESTADO != "alta" ORDER BY equipo.MARCA ASC`;

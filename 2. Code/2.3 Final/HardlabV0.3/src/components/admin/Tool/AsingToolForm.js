...
export function AsingToolForm() {
    ...
    const handleSubmit = event => {
        ...
        console.log(data)
        axios.get(`${URIS.HERRAMIENTA}${COD_HERRAMIENTA}`)
            .then(res => {
                setHerram(res.data[0]);
                if (CAN_ASIGNADA > res.data[0].CANT_DISPONIBLE) {
                    mostrarAlertaError();
                } else {
                    axios.post(URIS.ASIGNACION, data)
                        .then(res => {
                            console.log(res);
                            console.log(res.data);
                        })
                    axios.put(`${URIS.ASIGNARCANTIDAD}${COD_HERRAMIENTA}`, { 'CAN_ASIGNADA': CAN_ASIGNADA })
                        .then(res => {
                            mostrarAlertaExito();
                        });
                }
    ...
    useEffect(() => {
        fetch(URIS.HERRAMIENTASDISPONIBLES)
            .then((response) => response.json())
            .then((data) => {
                setHerramienta(data);
            });
